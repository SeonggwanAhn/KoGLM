...
class ConstructBlockStrategy:
    ...
    def sample_span_in_document(self, tokens, masked_lengths, rng):
        ...
        # print(f'{len(documents)}th documents - {len(documents)}, {documents}')
        # print(f'{len(documents)}th masked_lengths - {masked_lengths}')
        for i, (offset, length) in enumerate(documents):
            if i == len(documents) - 1:
                ...
                current_masked_length, current_count = 0, 0
                # print(f'{len(documents)}th (1) {current_masked_total} (2) {mask_index} (3) {current_count} (4) {len(masked_lengths)} (5) {masked_lengths[mask_index+current_count]+current_masked_length}')
                while mask_index + current_count < len(masked_lengths) and masked_lengths[
            ...
            # print(f'{len(documents)} mask_spans - {mask_spans}')
        return mask_spans
    ...
    def generate_blank_data(self, sample, masked_lengths, attention_mask, rng, cnt, task='bert'):
        rng.shuffle(masked_lengths)
        ...
        # print(f'{cnt}cnt block spans lengths - {block_spans}, {len(block_spans)}')
        # print(f'{cnt}cnt masked lengths - {masked_lengths}, {len(masked_lengths)}')
        if len(block_spans) < len(masked_lengths):
            # print(f'does it pass here?? block_spans - {len(block_spans)}, {len(masked_lengths)}')
            return None
        if self.masked_lm:
            data = self.make_masked_data(tokens, loss_masks, attention_mask, block_spans, rng)
        else:
            # print('this path is intended')
    ...
    def construct_blocks(self, samples):
        ...
        if rand < self.bert_prob:
            ...
            # print('bert mode')
            for sample in samples:
                # masked_lengths - how long to mask. maybe it should be one element...?
                if single_span:
                ...
                ## TODO: (1) check masked_lengths in samples var
                ## (2) is data continuously None?
                data = self.generate_blank_data(sample, masked_lengths, sep, rng, len(masked_lengths), task='bert')
                if data is not None:
                    if self.encoder_decoder:
        ...
        if self.encoder_decoder:
            ...
            token_batch, target_batch, loss_mask_batch, position_id_batch = self.pad_batch(token_batch, target_batch,
                                                                                           loss_mask_batch,
                                                                                           position_id_batch, len(masked_lengths))
    ...
    @staticmethod
    def pad_batch(token_batch, target_batch, loss_mask_batch, position_id_batch, cnt):
        ...
        """
            ...
            print(f'{cnt}cnt token_batch - ', token_batch)
            return None
        """
